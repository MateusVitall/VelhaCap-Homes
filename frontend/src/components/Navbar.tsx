    ...
    <header className="sticky top-0 z-40 border-b border-border/60 bg-background/85 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
        ...
        <nav className="hidden md:flex items-center gap-7 text-sm">
          <Link to="/" className="text-foreground/70 hover:text-foreground transition-colors" activeOptions={{ exact: true }} activeProps={{ className: "text-foreground" }}>
            Início
          </Link>
          <Link to="/imoveis" className="text-foreground/70 hover:text-foreground transition-colors" activeProps={{ className: "text-foreground" }}>
            Imóveis
          </Link>
          {user && (
            <Link to="/painel" className="text-foreground/70 hover:text-foreground transition-colors" activeProps={{ className: "text-foreground" }}>
              Meu painel
            </Link>
          )}
        </nav>
      ...
      {open && (
        <div className="md:hidden border-t border-border bg-background">
          <div className="mx-auto max-w-7xl px-4 py-3 flex flex-col gap-1">
            ...
            <Link to="/imoveis" onClick={() => setOpen(false)} className="py-2">Imóveis</Link>
            {user && <Link to="/painel" onClick={() => setOpen(false)} className="py-2">Meu painel</Link>}
            <div className="mt-2 flex gap-2">
              {user ? (
                <Button variant="outline" className="flex-1" onClick={() => { handleLogout(); setOpen(false); }}>
                  Sair
                </Button>
              ) : (
                <>
