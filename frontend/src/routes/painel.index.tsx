...
import { getMyProperties } from "@/lib/store";
...
function DashboardHome() {
  const { user } = useAuth();
  const props = user ? getMyProperties(user.id) : [];
  ...
  return (
    <div>
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <Link to="/painel/novo">
          <Button className="gap-2"><ListPlus className="h-4 w-4" /> Novo imóvel</Button>
        </Link>
      </div>

      <div className="mt-8 grid gap-4 sm:grid-cols-3">
        <Card icon={<Building className="h-5 w-5" />} label="Imóveis ativos" value={String(props.length)} />
      ...
      <div className="mt-10 rounded-xl border border-border bg-card p-8">
        ...
        <p className="mt-2 text-sm text-muted-foreground max-w-lg">
          Cadastre seu primeiro imóvel para começar a receber interessados. É grátis e leva poucos minutos.
        </p>
        <div className="mt-5 flex gap-3">
          <Link to="/painel/novo"><Button>Cadastrar imóvel</Button></Link>
          <Link to="/painel/imoveis"><Button variant="outline">Ver meus imóveis</Button></Link>
        </div>
      </div>
    </div>
  );
